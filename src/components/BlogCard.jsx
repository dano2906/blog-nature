...
export default function BlogCard({ img, alt, title, paragraph }) {
  ...
  return (
    <article
      ref={ref}
      className={`w-full sm:w-80 h-auto rounded-lg shadow transition-all shadow-green-950 bg-lime-200/100 border border-green-950 flex flex-col items-center justify-center ${
        inView ? `animate-in zoom-in-0 delay-100 duration-500` : "scale-0"
      }`}
    >
      <img
        src={img}
